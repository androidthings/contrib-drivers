...
import com.google.android.things.contrib.driver.cap12xx.Cap12xx.AlertCallback;
import com.google.android.things.contrib.driver.cap12xx.Cap12xx.Configuration;
import com.google.android.things.contrib.driver.cap12xx.Cap12xx.OnCapTouchListener;
...
@PrepareForTest({Cap12xx.class, Gpio.class})
public class Cap12xxTest {
    ...
    public void driverInit() throws IOException {
        ...
        verify(mI2c).writeRegByte(eq(0x22),
                byteThat(hasBitsSet((byte) (0x0F & Cap12xx.REPEAT_NORMAL))));
    ...
    public void setRepeatRate() throws IOException {
        ...
        mDriver.setRepeatRate(Cap12xx.REPEAT_SLOW);
        verify(mI2c).writeRegByte(0x28, (byte) 0xFF);
        verify(mI2c).writeRegByte(eq(0x22),
                byteThat(hasBitsSet((byte) (0x0F & Cap12xx.REPEAT_SLOW))));
        verify(mI2c).writeRegByte(eq(0x23),
                byteThat(hasBitsSet((byte) (0x0F & Cap12xx.REPEAT_SLOW))));

        Mockito.reset(mI2c);

        mDriver.setRepeatRate(Cap12xx.REPEAT_FAST);
        verify(mI2c).writeRegByte(0x28, (byte) 0xFF);
        verify(mI2c).writeRegByte(eq(0x22),
                byteThat(hasBitsSet((byte) (0x0F & Cap12xx.REPEAT_FAST))));
        verify(mI2c).writeRegByte(eq(0x23),
                byteThat(hasBitsSet((byte) (0x0F & Cap12xx.REPEAT_FAST))));
    }
    ...
    public void setSensitivity() throws IOException {
        mDriver.setSensitivity(Cap12xx.SENSITIVITY_LOW);
        verify(mI2c).writeRegByte(eq(0x1F),
                byteThat(hasBitsSet((byte) (0x70 & Cap12xx.SENSITIVITY_LOW))));
